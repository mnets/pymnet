...
class AuxiliaryGraphBuilderNX(isomcore.AuxiliaryGraphBuilder):
    ...
    def compare_structure(self, other):
        def matcher(n1, n2): return n1['color'] == n2['color']
        return networkx.is_isomorphic(self.nxgraph, other.nxgraph,
                                      node_match=matcher)

    def _isomorphism_mapping(self, other):
        def matcher(n1, n2): return n1['color'] == n2['color']
        m = nxisomorphism.GraphMatcher(
            self.nxgraph, other.nxgraph, node_match=matcher)
