    ...
    def draw(self, **kwargs):
        ...
        if ax is None:
            ...
            assert isinstance(ax, Axes3D), \
                "The axes need to have 3D projection. Use, for example, " \
    ...
    def draw(self):
        ...
        if self.label is not None:
            self.labelObject = self.net.ax.text(
                ...
                self.layer.z+self.net.eps, str(self.label), **self.labelArgs)
    ...
    def draw(self):
        ...
        if self.shape == "rectangle":
            self.layer = Rectangle(
                (0, 0), 1, 1, alpha=self.alpha, color=self.color)
            if self.label is not None:
                self.labelObject = self.net.ax.text(
                    self.labelloc[0], self.labelloc[1],
                    self.z, str(self.label), **self.labelArgs)
        elif self.shape == "circle":
            self.layer = Circle(
                (0.5, 0.5), 0.5, alpha=self.alpha, color=self.color)
            if self.label is not None:
                self.labelObject = self.net.ax.text(
                    self.labelloc[0], self.labelloc[1],
                    self.z, str(self.label), **self.labelArgs)
...
class EdgeMPL(drawnet.Edge):
    def draw(self):
        ...
        if abs(self.node1.layer.z - self.node2.layer.z) > self.net.layergap:
            n = int(round(abs(self.node1.layer.z - self.node2.layer.z) /
                    float(self.net.layergap)))+1
            import numpy
        ...
        for i in range(len(zs)-1):
            ...
            line = self.net.ax.plot(xs[i:i+2], ys[i:i+2], zs=zs[i:i+2],
                                    linestyle=self.style, zdir="z",
